...
class analyser {
    ...
    private function main_analyser() {
        ...
        // NULL, means it's first crawling of current problem
            ...
            $this->last_record_id[$this->problem_id] = $last_record_id;
            $last_record_id = NULL;
        }
        
        // still is on first page
        if ($last_record_id !== NULL AND strcmp($this->last_record_id[$this->problem_id], $last_record_id) === 0) {
            $total_pages = 1;
        }
        ...
        foreach ($this->result as $user_name => $user_result) {
            $this->result[$user_name]["try"] = $this->temp_try[$user_name];
        }
    }
    ...
    private function result_extractor($row) {
        ...
        if (!isset($this->result[$user_name_hashed]) OR $this->result[$user_name_hashed]['result'] !== TRUE) {
            ...
            if (isset($this->result[$user_name_hashed])) {
                if ($this->result[$user_name_hashed]['result'] !== TRUE) {
                    $this->result[$user_name_hashed]['result'] = $user_result;
                    if($this->result[$user_name_hashed]['time'] < $user_submit_time)
                    {
